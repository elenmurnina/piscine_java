...
public class MessagesRepositoryJdbcImpl implements MessagesRepository {
    ...
    public void update(Message message) throws SQLException {
        final String QUERY = "UPDATE chat.messages SET author = ?, room = ?, textmsg = ?, datetime = ? WHERE id = ?";
        ...
        try (Connection connection = ds.getConnection();
            ...
            pStatement.setLong(2, message.getRoom().getId());
            pStatement.setString(3, message.getText());
            pStatement.setTimestamp(4, message.getMessageDateTime() == null
                    ? null : Timestamp.valueOf(message.getMessageDateTime()));
            pStatement.setLong(5, message.getId());
            pStatement.executeUpdate();
        }
    }
}
